...
function addRows() {
    ...
    tbody.append(newTr);
    if (rowLengthForAdd + 1 >= 10) appendRow.disabled = true;
...
function removeRows() {
    ...
    if (rowLengthFroRemove - 1 <= 2) removeRow.disabled = true;
...
function appendColumns() {
    ...
    if (tdCountForAdd + 1 >= 10) appendColumn.disabled = true;
    removeColumn.disabled = false;
}
// видаляємо колонки!
function removeColumns() {
    const tdCountForRemove = document.querySelectorAll("tr:first-child td").length;
    ...
    if (tdCountForRemove - 1 <= 2) removeColumn.disabled = true;
...
removeRow.addEventListener("click", (e)=>{
    if (e.target) removeRows();
});
appendColumn.addEventListener("click", (e)=>{
    if (e.target) appendColumns();
});
removeColumn.addEventListener("click", (e)=>{
    if (e.target) removeColumns();
});
